...
while beat_game != True:
    ...
    # Randomly select special rooms without repetition
    available_rooms = room_options.copy()
    
    # Final boss room (creature that attacked player in opening story)
    boss_room = random.choice(available_rooms)
    available_rooms.remove(boss_room)

    # Exit room (escape route from dungeon)
    exit_room = random.choice(available_rooms)
    available_rooms.remove(exit_room)
    
    # Treasure chest rooms (rewards for exploration)
    ...
    # Mini-boss room (tougher encounter before final boss)
    mini_boss_room = random.choice(available_rooms)
    available_rooms.remove(mini_boss_room)

    # fight room locations (multiple rooms with combat encounters)
    num_fight_rooms = min(3, len(available_rooms))  # At least 3 fight rooms
    fight_room_locations = set(random.sample(available_rooms, num_fight_rooms))
    for room in fight_room_locations:
        available_rooms.remove(room)

    # heal room locations (multiple rooms with healing fountains)
    num_heal_rooms = min(2, len(available_rooms))  # At least 2 heal rooms
    heal_room_locations = set(random.sample(available_rooms, num_heal_rooms))
    for room in heal_room_locations:
        available_rooms.remove(room)

    # empty room locations (rooms with no special encounters)
